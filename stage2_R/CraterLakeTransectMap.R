...
CraterLake_80transects <- read.csv(file="CraterLake_80transects.csv",
                                   header=TRUE, sep=",")
...
CraterLake_Joy <- ggplot(CraterLake_80transects, 
                         aes(x = Lon, y = Lat, group = Lat, height = Elev)) + 
  geom_density_ridges(stat = "identity", scale = 15,
...
# Customized plot: 
ggplot(CraterLake_transects,
       aes(x = Lon, y = Lat, group = Lat, height = Elev)) + 
  geom_density_ridges(stat = "identity", scale = 15,
                      fill="pink", color = "violetred4", size = 1, linetype = "12") +
  scale_x_continuous(name = "CRATER LAKE NATIONAL PARK")  +
  ylim(42.77, 43.15) +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
